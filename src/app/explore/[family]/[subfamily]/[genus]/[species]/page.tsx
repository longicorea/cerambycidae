...
function LabelText({label, value}: { label: string, value: string | number }) {
    return (
        <div className={"grid grid-cols-4 items-center space-x-2"}>
            <span className={"col-span-1 w-20 text-slate-700 font-medium"}>{label}</span>
            <span
                className={"col-span-3 font-light text-slate-600 text-sm overflow-hidden text-nowrap text-ellipsis"}>{value}</span>
        </div>
    )
}
